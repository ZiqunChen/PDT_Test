...
def local_train(model, dataloader):
    ...
    if not os.path.exists('figs/FL_local_loss'):
        os.makedirs('figs/FL_local_loss', exist_ok=True)
    if not os.path.exists('FL_local_models'):
        os.makedirs('FL_local_models', exist_ok=True)
    ...
    # local_model = Transformer(config).to(device)
    # local_model.load_state_dict(model)
    ...
    optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1e-4)
    loss_fn = torch.nn.CrossEntropyLoss(reduction='sum')
    print("Num train batches: " + str(len(dataloader)))
    ...
    for epoch in range(num_epochs):
        ...
        for i, batch in enumerate(dataloader):
            print(f"Batch {i} of {len(dataloader)}", end='\r')
            batch = {k: v.to(device) for k, v in batch.items()}
            true_actions = batch['optimal_actions']
            pred_actions = model(batch)
            true_actions = true_actions.unsqueeze(
    ...
    # torch.save(model.state_dict(), f'FL_local_models/{filename}.pt')
